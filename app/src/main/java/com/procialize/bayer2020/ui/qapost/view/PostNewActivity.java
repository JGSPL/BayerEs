                ...
                if (cd.isConnectingToInternet()) {
                    ...
                    if (resultList.size() == 0) {
                        ...
                        try {
                            ...
                            RequestBody mPost_content = RequestBody.create(MediaType.parse("text/plain"), postText);
                            ...
                            for (int i = 0; i < resultList.size(); i++) {
                                ...
                                MultipartBody.Part filePart = MultipartBody.Part.createFormData("media_file[]", fileName + ".png", RequestBody.create(MediaType.parse("image/png"), file));
                                parts.add(filePart);
                               /* File file ;
                                MultipartBody.Part filePart = null;


                               // if (MediaType.parse(resultList.get(i).getmMimeType()) == null) {
                                    if (resultList.get(i).getmMediaType().equalsIgnoreCase("image")) {
                                        filePart = MultipartBody.Part.createFormData("media_file[]", resultList.get(i).getmPath(), RequestBody.create(MediaType.parse("image/png"), resultList.get(i).getmPath()));
                                    } *//*else {
                                        filePart = MultipartBody.Part.createFormData("media_file[]", resultList.get(i).getmPath(), RequestBody.create(MediaType.parse("video/mp4"), resultList.get(i).getmPath()));
                                    }*//*
                                *//*} else {
                                    filePart = MultipartBody.Part.createFormData("media_file[]", file.getName(), RequestBody.create(MediaType.parse(resultList.get(i).getmMimeType()), file));
                                }*//*
                                parts.add(filePart);*/
                            }
                            ...
                            /*if (newsfeedApi.postNewsFeed1(api_token, mevent_id, mPost_content, parts, thumbParts).isExecuted())
                                newsfeedApi.postNewsFeed1(api_token, mevent_id, mPost_content, parts, thumbParts).cancel();*/

                            newsfeedApi.postNewsFeed1(api_token, mevent_id, mPost_content, parts, thumbParts)//,Media_file,Media_file_thumb)
                                        ...
                                        public void onResponse(Call<LoginOrganizer> call, final Response<LoginOrganizer> response) {
                                            if (response.isSuccessful()) {
                                                ...
                                                if (response != null) {
                                                    ...
                                                    Utility.createLongSnackBar(linear, "failure");
                                                }
                                            }
                                        }
